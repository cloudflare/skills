...
11. **Persistence, only when requested.** If this skill came from an installed plugin or a persistent skills directory, it is already reusable; skip this step. Do not create a second installation by default. If the user requests a project-local copy, choose the active client's supported skill directory (or the user's explicit destination), then run this loaded bundle's `scripts/persist-skill.sh --path <bundle-directory>/SKILL.md` from the project root. The helper copies the executing bundle, requires an empty destination inside the project, and does not fetch a newer revision. A project copy has its own update lifecycle; explain that when creating it. For a file-oriented rules target or a hosted prompt without local bundle files, follow the hosted prompt's bootstrap instructions instead of using this helper.
