...
| Run locally or choose a testing approach | [Local development](https://developers.cloudflare.com/workers/local-development/) |
...
- Prefer `wrangler.jsonc` for new config. Preserve existing project conventions; avoid incidental format migrations or compatibility-date changes.
...
- Keep secret values out of command arguments, source, and logs. Use the documented interactive input or protected file/stdin mechanism for the command. Local secret files must be ignored by version control and do not configure deployed secrets.
