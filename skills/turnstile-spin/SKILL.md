...
| Existing Cloudflare Pages project              | Wire siteverify inside a Pages Function (or the equivalent for the existing framework). The [Turnstile Pages Plugin](https://developers.cloudflare.com/pages/functions/plugins/turnstile/) remains a valid shortcut. For a new Cloudflare application, prefer a Worker and the canonical direct Siteverify flow. |
