...
- For an existing supported framework app or static site without Wrangler config, prefer [automatic configuration](https://developers.cloudflare.com/workers/framework-guides/automatic-configuration/) before manually writing config or installing adapters: `wrangler setup` configures only (`--dry-run` previews changes), while `wrangler deploy` configures and deploys. Wrangler 4.68+ needs no `--x-autoconfig` flag.
