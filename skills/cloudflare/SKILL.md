...
| Start a Worker or framework project | Framework-native tooling or the Workers CLI | Use the framework's setup tooling for framework apps; use the Workers CLI for a basic Worker | [frameworks-on-cloudflare skill](../frameworks-on-cloudflare/SKILL.md); [Workers CLI guide](https://developers.cloudflare.com/workers/get-started/guide/); `wrangler` skill |
